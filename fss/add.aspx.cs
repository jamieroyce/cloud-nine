	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			if (String.IsNullOrEmpty(scheduled)){
				...
				{
					String query = "INSERT into bis(reg_cat_id, addo_id, name, status, service, area, scheduled, weekend, org, notes) "
								 + "VALUES (@reg_cat_id, @addo_id, @name, @status, @service, @area, getdate(), getdate(), @org, @notes)";
								 
					using(SqlCommand command = new SqlCommand(query, connection))
					{
						command.Parameters.AddWithValue("@reg_cat_id",		"First Service");
				...
				using(SqlConnection connection = databaseConnection.CreateSqlConnection())
				{
					...
					using(SqlCommand command = new SqlCommand(query, connection))
					{
						...
						Response.Redirect("./firststarts.aspx");
