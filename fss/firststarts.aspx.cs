	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			if(searchCol != "" && searchText != "")
				...
				DataGrid_Load(DAL.get_log("First Service", OrgText.Text), "reg");
		} 
    }
	...
	public void Archive_Click(object sender, EventArgs e)
	{
		...
		if ( clickedButton != null)
		{
			...
			foreach (GridViewRow row in gv.Rows)
			{
				...
				if(we != null && we != "" && we != " " && nameStr != " "  && nameStr != "" && nameStr != null && areaStr != " "  && areaStr != "" && areaStr != null)
				{																	
	...
	{	
		...
		try // 
		{
			var query = from c in dataTable.AsEnumerable() 
					   where c.Field<string>("status") == "First Service" && c.Field<string>("reg_cat_id") == "First Service"
					  select c ;	
