...
{  	
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			string area = String.Format("{0}", 		Request.Form["areaid"]);	
			string line = String.Format("{0}", 		Request.Form["lineid3"]);				
			string rank = String.Format("{0}", 		Request.Form["ddlReg_Addo"]);	
			...
			if (String.IsNullOrEmpty(scheduled)){
				...
				{
					String query = "INSERT into bis(reg_cat_id, addo_id, name, rank, service, status, line, scheduled,  weekend, org, area, notes) "
								 + "VALUES (@reg_cat_id, @addo_id, @name, @rank, @service, @status, @line,  getdate(), getdate(), @org, @area, @notes)";
				...
				using(SqlConnection connection = databaseConnection.CreateSqlConnection())
				{
					String query = "INSERT into bis(reg_cat_id, addo_id, name, rank, service, status,  line, scheduled, org, area, notes) "
								 + "VALUES (@reg_cat_id, @addo_id, @name, @rank, @service, @status,  @line, @scheduled, @org, @area, @notes)";
