...
{  	
	...
	public void StartingPage()
	{
		...
		GridView_Load(GridViewEvent, DAL.getEvents()); 		
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			{
				String query = "INSERT into listReg(full_name, short_name, post) "
							 + "VALUES (@full_name, @short_name, @post)";
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			using(SqlConnection connection = databaseConnection.CreateSqlConnection())
			{
				String query = "INSERT into event(name, event_desc, event_location, attendance_quota, event_date, event_start, event_end, notes, org_id, date_created, date_modified) "
							 + "VALUES (@name, @event_desc, @event_location, @attendance_quota, @event_date, @event_start, @event_end, @notes, @org_id, getdate(), getdate())";
	...
	protected void text_change(object sender, EventArgs e)
	{
		...
		GridViewRow gvRow = (GridViewRow)(sender as Control).Parent.Parent;
		string id = gvRow.Cells[0].Text;
		string sqlCommandStatement =  string.Format("UPDATE event SET {0} = @TEXT WHERE id=@ID", text.ID);
		SqlCmd(sqlCommandStatement, id, text.Text);		
