...
{  	
	...
	public void StartingPage()
	{
		...
		GridView_Load(GridViewReg, DAL.getExpectancy(OrgText.Text)); 
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			string area = String.Format("{0}", 		Request.Form["shortnameID"]);	
			string quota = String.Format("{0}", 	Request.Form["quotaID"]);	
			string org = String.Format("{0}", 			Request.Form["postID"]);	
	...
	protected void Selection_Change_Org(object sender, EventArgs e)
	{
		...
		GridViewRow gvRow = (GridViewRow)(sender as Control).Parent.Parent;
		string id = gvRow.Cells[0].Text;
		string sqlCommandStatement = "UPDATE lookup SET desc3 = @TEXT WHERE id=@ID";
