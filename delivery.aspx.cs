...
{  	
	...
	public void Day_Click(Object sender, EventArgs e)
	{
		...
		ShieldChartPurif.DataBind();
		ShieldChartDIV6.DataBind();
		ShieldChartHGC.DataBind();
		ShieldChartSRD.DataBind();
		ShieldChartACAD.DataBind();
		ShieldChartGAK.DataBind();
    ...
    protected void LinkButtonSignUp_Command(Object sender, EventArgs e) 
    {
		var v = ((LinkButton)sender).CommandArgument;
		Response.Redirect("gi/log.aspx?filter=" + v );

	}

    protected void LinkButtonStart_Command(Object sender, EventArgs e) 
    {
		var v = ((LinkButton)sender).CommandArgument;
		Response.Redirect("starts/log.aspx?filter=" + v );

	}

    protected void LinkButtonBIS_Command(Object sender, EventArgs e) 
    {
		var v = ((LinkButton)sender).CommandArgument;
		Response.Redirect("bis/log.aspx?filter=" + v );

	}

    protected void LinkButtonComp_Command(Object sender, EventArgs e) 
    {
		var v = ((LinkButton)sender).CommandArgument;
		Response.Redirect("comprsn/log.aspx?filter=" + v );
...
/*LAST WEEK		*/
	...
	{ 
		...
		DataTable dataThisWeek = new DataTable();
		dataThisWeek = DAL.GetOrgDailyCategory("thisweek", "Purif", "Purif Start", OrgText.Text);

		DataTable thisWeekSched = new DataTable();
		thisWeekSched = DAL.GetOrgDailyCategory("thisweeksched", "Purif", "Purif Start", OrgText.Text);

		DataTable dataLastWeek = new DataTable();
		dataLastWeek = DAL.GetOrgDailyCategory("lastweek", "Purif", "Purif Start", OrgText.Text);
...
/*LAST WEEK		*/
	...
	protected void ShieldChartPurif_TakeDataSource(object sender, Shield.Web.UI.ChartTakeDataSourceEventArgs e) 
	{ 
		DataTable dt = new DataTable();
		dt = DAL.GetDeliveryBar(OrgText.Text, "PURIF");
		// ErrorText.Text += "OrgText=" + OrgText.Text + "</BR>";

		//EXPECTANCY
		DataTable dtExpect = new DataTable();
		dtExpect = DAL.getExpectancy(OrgText.Text);
		
		string quota = "0";
		foreach(DataRow row in dtExpect.Rows)
		{
			if(row.Field<string>("desc1")=="purif")
				quota = row.Field<string>("desc2");
		}			

		try{
			if(dt.Rows.Count>0){
				ShieldChartPurif.SecondaryHeader.Text = "Expectancy: " + quota;
				ShieldChartPurif.DataSource = dt;
			} else {
				ShieldChartPurif.SecondaryHeader.Text = "(No Data Found)";
			}
		}
		catch(Exception ex){
	...
	protected void ShieldChartDIV6_TakeDataSource(object sender, Shield.Web.UI.ChartTakeDataSourceEventArgs e) 
	{ 
		DataTable dt = new DataTable();
		dt = DAL.GetDeliveryBar(OrgText.Text, "DIV6");
		// ErrorText.Text += "OrgText=" + OrgText.Text + "</BR>";
		...
		foreach(DataRow row in dtExpect.Rows)
		{
			if(row.Field<string>("desc1")=="div6")
				quota = row.Field<string>("desc2");
		}			

		try{
			if(dt.Rows.Count>0){
				ShieldChartDIV6.SecondaryHeader.Text = "Expectancy: " + quota;
				ShieldChartDIV6.DataSource = dt;
			} else {
				ShieldChartDIV6.SecondaryHeader.Text = "(No Data Found)";
			}
		}
		catch(Exception ex){
			ErrorText.Text += "ERROR: " + ex.ToString();
	...
	protected void ShieldChartHGC_TakeDataSource(object sender, Shield.Web.UI.ChartTakeDataSourceEventArgs e) 
	{ 
		DataTable dt = new DataTable();
		dt = DAL.GetDeliveryBar(OrgText.Text, "HGC");
		// ErrorText.Text += "OrgText=" + OrgText.Text + "</BR>";
		...
		foreach(DataRow row in dtExpect.Rows)
		{
			if(row.Field<string>("desc1")=="hgc")
				quota = row.Field<string>("desc2");
		}			

		try{
			if(dt.Rows.Count>0){
				ShieldChartHGC.DataSource = dt;
				ShieldChartHGC.SecondaryHeader.Text = "Expectancy: " + quota;
			} else {
				ShieldChartHGC.SecondaryHeader.Text = "(No Data Found)";
			}
		}
		catch(Exception ex){
			ErrorText.Text += "ERROR: " + ex.ToString();
	...
	protected void ShieldChartSRD_TakeDataSource(object sender, Shield.Web.UI.ChartTakeDataSourceEventArgs e) 
	{ 
		DataTable dt = new DataTable();
		dt = DAL.GetDeliveryBar(OrgText.Text, "SRD");
		// ErrorText.Text += "OrgText=" + OrgText.Text + "</BR>";
		...
		foreach(DataRow row in dtExpect.Rows)
		{
			if(row.Field<string>("desc1")=="srd")
				quota = row.Field<string>("desc2");
		}			

		try{
			if(dt.Rows.Count>0){
				ShieldChartSRD.DataSource = dt;
				ShieldChartSRD.SecondaryHeader.Text = "Expectancy: " + quota;
			} else {
				ShieldChartSRD.SecondaryHeader.Text = "(No Data Found)";
			}
		}
		catch(Exception ex){
			ErrorText.Text += "ERROR: " + ex.ToString();
	...
	protected void ShieldChartACAD_TakeDataSource(object sender, Shield.Web.UI.ChartTakeDataSourceEventArgs e) 
	{ 
		DataTable dt = new DataTable();
		dt = DAL.GetDeliveryBar(OrgText.Text, "ACAD");
		...
		foreach(DataRow row in dtExpect.Rows)
		{
			if(row.Field<string>("desc1")=="acad")
				quota = row.Field<string>("desc2");
		}			
				 
		// ErrorText.Text += "quota=" + quota + "</BR>";
		try{
			if(dt.Rows.Count>0){
				ShieldChartACAD.DataSource = dt;
				ShieldChartACAD.SecondaryHeader.Text = "Expectancy: " + quota;
			} else {
				ShieldChartACAD.SecondaryHeader.Text = "(No Data Found)";
			}
		}
		catch(Exception ex){
			ErrorText.Text += "ERROR: " + ex.ToString();
	...
	protected void ShieldChartGAK_TakeDataSource(object sender, Shield.Web.UI.ChartTakeDataSourceEventArgs e) 
	{ 
		DataTable dt = new DataTable();
		dt = DAL.GetDeliveryBar(OrgText.Text, "GAK");
		// ErrorText.Text += "OrgText=" + OrgText.Text + "</BR>";
		...
		foreach(DataRow row in dtExpect.Rows)
		{
			if(row.Field<string>("desc1")=="gak")
				quota = row.Field<string>("desc2");
		}			
		
		try{
			if(dt.Rows.Count>0){
				ShieldChartGAK.DataSource = dt;
				ShieldChartGAK.SecondaryHeader.Text = "Expectancy: " + quota;
			} else {
				ShieldChartGAK.SecondaryHeader.Text = "(No Data Found)";
			}
		}
		catch(Exception ex){
			ErrorText.Text += "ERROR: " + ex.ToString();
