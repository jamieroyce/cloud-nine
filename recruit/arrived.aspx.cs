...
{  	
	...
	public void Archive_Click(object sender, EventArgs e)
	{
		...
		if ( clickedButton != null)
		{
			...
			int i = 0;
			GridView gv = (GridView)(sender as Control).Parent.FindControl("GridViewArrived");		
			foreach (GridViewRow row in gv.Rows)
			{
				...
				dt = (TextBox)row.FindControl("reg");
				if ( dt != null) 
					regStr = dt.Text;

				
				if(we != null && we != "" && we != " " && nameStr != " "  && nameStr != "" && nameStr != null && regStr != " "  && regStr != "" && regStr != null)
				{																	
					...
					if(we == null || we == "" || we == " ")
						...
						ErrorText.Text += "Row " + i + " is Incomplete [" + nameStr + " / " + regStr + "] (Name and Recruit are required Fields)<br />";
				}
			}
			LoadLastWeek(we);
		}
			...
			using(SqlConnection connection = databaseConnection.CreateSqlConnection())
			{
				...
				string sqlCommandStatement = 
					String.Format( "INSERT INTO bis (addo_id, name, area, service, reg, fsm, phone, email, org, status, line, reg_cat_id, last_week_bis, consecutive_weeks, notes) "
								...
								+ "FROM bis "
								+ "WHERE weekend = '{0}' and org = 'Fdn' and reg_cat_id = 'Archive' "
								+ "AND addo_id not in ( select addo_id from bis where reg_cat_id = 'LineUp' and org = 'Fdn') ", weekend);
				try
				{		
					using (SqlConnection conn = databaseConnection.CreateSqlConnection())
					{
				...
				string sqlCommandStatement2 = 
					String.Format( "INSERT INTO bis (addo_id, name, area, service, reg, fsm, phone, email, org, status, line, reg_cat_id, last_week_bis, consecutive_weeks, notes) "
								+ "SELECT addo_id, name, area, service, reg, fsm, phone, email, org, 'Named' as  status, line, 'LineUp' as reg_cat_id, 1 as last_week_bis, isnull(consecutive_weeks,0) + 1 as consecutive_weeks, cast(isnull(consecutive_weeks,0) + 1 AS varchar(10))  + ' consecutive weeks ' as note "
								+ "FROM bis "
								+ "WHERE weekend = '{0}' and org = 'Day' and reg_cat_id = 'Archive' "
								+ "AND addo_id not in ( select addo_id from bis where reg_cat_id = 'LineUp' and org = 'Day')", weekend);
				try
				{		
					using (SqlConnection conn = databaseConnection.CreateSqlConnection())
					{
	...
	{
		GridViewArrived.PageIndex = e.NewPageIndex;
	...
	protected void Rank_Change(object sender, EventArgs e)
	{
		...
		if(searchCol != "" && searchText != "")
			DataGrid_Load(DAL.Search_BIS(status, OrgText.Text, searchText, searchCol), "reg");
		else
			DataGrid_Load(DAL.bis(status, OrgText.Text), "reg");
	...
	{	
		...
		try // Arrived
		{
			var query = from c in dataTable.AsEnumerable() 
					   where c.Field<string>("status") == status && c.Field<string>("reg_cat_id") == "Recruit"
					  select c ;	
