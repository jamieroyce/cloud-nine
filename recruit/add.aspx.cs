...
{  	
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			{
				String query = "INSERT into bis(reg_cat_id, addo_id, name, status, reg, scheduled, weekend, org, notes) "
							 + "VALUES (@reg_cat_id, @addo_id, @name, @status, @reg, getdate(), getdate(), @org, @notes)";
							 
				using(SqlCommand command = new SqlCommand(query, connection))
				{
					command.Parameters.AddWithValue("@reg_cat_id",		"Recruit");
