...
{  	
	...
	public void StartingPage()
	{
		...
		Title = "Fallen Off BIS";
		HeadText.Text = Title;
		HeaderText.Text = "Last Week BIS";
		status = "Last Week";
	...
	{
		GridViewLastWeek.PageIndex = e.NewPageIndex;
	...
	{	
		...
		try // 
		{
			var query = from c in dataTable.AsEnumerable() 
					   where c.Field<string>("status") == "Last Week" && c.Field<string>("reg_cat_id") == "LineUp"
					  select c ;	
			...
			var cmdNamed 	 = "SELECT count(distinct org + name) AS total from bis WHERE status = 'Last Week' and reg_cat_id = 'LineUp'";
			var cmdNamedD	 = "SELECT count(distinct org + name) AS total from bis WHERE status = 'Last Week' and reg_cat_id = 'LineUp' and org = 'Day'";
			var cmdNamedFdn  = "SELECT count(distinct org + name) AS total from bis WHERE status = 'Last Week' and reg_cat_id = 'LineUp' and org = 'Fdn'";
			...
			if(OrgText.Text=="Combined"){
				AmountText.Text = ResultNamed;
			} else if(OrgText.Text =="Day"){
				AmountText.Text = ResultDayNamed;
			} else {
				AmountText.Text = ResultFdnNamed;
			}
