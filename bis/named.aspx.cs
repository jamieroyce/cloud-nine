...
{  	
	...
	public void StartingPage()
	{
		...
		HeadText.Text = Title;
		HeaderText.Text = "Named/Fallen Off BIS";
		status = "Named";
	...
	{
		GridViewNamed.PageIndex = e.NewPageIndex;
	...
	{	
		...
		try // 
		{
			var query = from c in dataTable.AsEnumerable() 
					   where c.Field<string>("status") == "Named" && c.Field<string>("reg_cat_id") == "LineUp"
					  select c ;	
			...
			var cmdNamedD	 	 = "SELECT count(distinct org + name) AS total from bis WHERE status = 'Named' and reg_cat_id = 'LineUp' and org = 'Day'";
			var cmdNamedFdn 	 = "SELECT count(distinct org + name) AS total from bis WHERE status = 'Named' and reg_cat_id = 'LineUp' and org = 'Fdn'";
