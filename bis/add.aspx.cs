...
{  	
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			string phone = String.Format("{0}", 	Request.Form["addo_phoneid"]);	
			...
			if (String.IsNullOrEmpty(scheduled)){
				...
				{
					String query = "INSERT into bis(reg_cat_id, addo_id, name, status, service, area, scheduled, weekend, org, phone, notes) "
								 + "VALUES (@reg_cat_id, @addo_id, @name, @status, @service, @area, getdate(), getdate(), @org, @phone, @notes)";
				...
				{
					String query = "INSERT into bis(reg_cat_id, addo_id, name,  status, service, area, scheduled, weekend,  org, phone, notes) "
								 + "VALUES (@reg_cat_id, @addo_id, @name,  @status, @service, @area, @scheduled, getdate(), @org, @phone, @notes)";
	...
	protected void ViewAddo(object sender, EventArgs e)
	{
        ...
        string phone = Server.HtmlDecode(gvRow.Cells[4].Text);
		
		addonameid.Text = name;
		addo_addoid.Text = id;
		addo_phoneid.Text = phone;
