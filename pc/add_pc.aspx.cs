...
{  	
	...
	{				
		...
		if ( clickedButton != null)
		{
			...
			string gak = String.Format("{0}", 		Request.Form["gak"]);	
			string has_meter = String.Format("{0}",	Request.Form["has_meter"]);	
			...
			{
				String query2 = "INSERT into pc (addo_id, name, org, case_lvl, trng_lvl, languages, gak_course_on, has_meter, note) "
							 + "VALUES (@addo_id, @name, @org, @case_lvl, @trng_lvl, @languages, @gak_course_on, @has_meter, @note)";
				...
				using(SqlCommand command = new SqlCommand(query2, connection))
				{
					...
					command.Parameters.AddWithValue("@gak_course_on",	gak);
					command.Parameters.AddWithValue("@has_meter",		has_meter);
					...
					ClearAddoModal();
					Response.Redirect("setup_pc.aspx");
	...
	protected void ViewAddo(object sender, EventArgs e)
	{
		...
		ClientScript.RegisterStartupScript(this.GetType(), "Pop", "openAddPC();", true);
